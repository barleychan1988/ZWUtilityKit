...
- (void)showAccessoryImage:(BOOL)bShow image:(nonnull UIImage *)image;
...
UIKIT_EXTERN NSString *__nonnull const ZWLabelTipCellID;

@interface ZWLabelTipCell : ZWSeperatorLineCell

@property (nonatomic, retain, readonly, nonnull)UILabel *labelTip;
@property (nonatomic, assign)VerticalAlignment alignmentTipVertical;
@property (nonatomic, assign)CGFloat fWidthTip;

//是否现在必填项提示符（红色*）
- (void)showRequiredTip:(BOOL)bShow;
- (void)showAccessory:(BOOL)bShow image:(nonnull NSString *)strImage;

@end

UIKIT_EXTERN NSString *__nonnull const ZWLabelLabelCellID;

@interface ZWLabelLabelCell : ZWLabelTipCell
@property (nonatomic, retain, readonly, nonnull)UILabel *labelContent;
...
UIKIT_EXTERN NSString *__nonnull const ZWLabelTFCellID;

@interface ZWLabelTFCell : ZWLabelTipCell

@property (nonatomic, retain, readonly, nonnull)UITextField *tfContent;
